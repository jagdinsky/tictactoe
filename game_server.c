...
int main(int argc, char** argv) {
    ...
    char field[9], stop = 's', go = 'g', won = 'w', lost = 'l', draw = 'd';
    int result;
    for (int i = 0; i < 9; i++)
        field[i] = ' ';
    for (int i = 0; ; i++, i %= 2) {
        ...
        if (result == 2) {
            write(client_socket[0], &lost, 1);
            write(client_socket[1], &won, 1);
            break;
        }
        if (result == 3) {
            write(client_socket[0], &draw, 1);
            write(client_socket[1], &draw, 1);
            break;
        }
    }
    return OK;
